...
if __name__ == '__main__':
    ...
    for i in f:
        i=i.splitlines()
        url = 'http://gwgp-pmhuvxwkzdv.n.bdcloudapi.com/verify_id_name'
        body='id_number=%s&name=%s'%(i[0],random_name())
        body=body.encode()
        print(body)

        headers = {
            'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8',
            'X-Bce-Signature': 'AppCode/f4f3c3fe2c714a268b6fb205d4ca8161'
        }
        r = requests.post(url=url,data=body,headers=headers)
        rhex=r.content.decode()
        rjson=demjson.decode(rhex)
        print(rjson)
        a+=1
        if rjson['result_message']=='一致':
            print(i,rjson['result_message'],a)
        else:
            print(rjson['result_message'],a)
        if a==11:
            break
